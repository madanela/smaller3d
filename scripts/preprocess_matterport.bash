...
function preprocess() {
    poetry run \
        python smaller3d/datasets/preprocessing/matterport_preprocessing.py preprocess_sequential \
            --data_dir="$DATA_DIR" \
            --save_dir="$SAVE_DIR" \
            --git_repo="$GIT_REPO" \
            --label_db="$LABEL_DB"
...
function make_instance_database() {
    poetry run \
        python smaller3d/datasets/preprocessing/matterport_preprocessing.py make_instance_database_sequential \
...
function preprocess_dvc () {
    python -m dvc run \
        -d $LABEL_DB \
        -d smaller3d/datasets/preprocessing/matterport_preprocessing.py \
        -o "${SAVE_DIR}/train" \
        ...
        \
        poetry run \
            python smaller3d/datasets/preprocessing/matterport_preprocessing.py preprocess \
            --data_dir="$DATA_DIR" \
            --save_dir="$SAVE_DIR" \
            --git_repo="$GIT_REPO" \
            --label_db="$LABEL_DB"
...
function make_instance_database_dvc () {
    python -m dvc run \
        ...
        \
        poetry run \
            python smaller3d/datasets/preprocessing/matterport_preprocessing.py make_instance_database \
            --train_database_path="$SAVE_DIR/train_database.yaml" \
            --git_repo="$GIT_REPO" \
            --data_dir="$DATA_DIR" \
            --save_dir="$SAVE_DIR" \
