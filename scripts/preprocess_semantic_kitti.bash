...
function preprocess() {
    poetry run \
        python smaller3d/datasets/preprocessing/semantic_kitti_preprocessing.py preprocess_sequential \
        --data_dir="$DATA_DIR" \
        --git_repo="$GIT_REPO" \
        --save_dir="$SAVE_DIR"
}
...
function make_instance_database() {
    poetry run \
        python smaller3d/datasets/preprocessing/semantic_kitti_preprocessing.py make_instance_database_sequential \
        --data_dir="$DATA_DIR" \
        --git_repo="$GIT_REPO" \
        --save_dir="$SAVE_DIR"
}
...
function preprocess_dvc () {
    python -m dvc run \
        -d smaller3d/datasets/preprocessing/semantic_kitti_preprocessing.py \
        -o "${SAVE_DIR}/train" \
        -o "${SAVE_DIR}/test" \
...
function make_instance_database_dvc () {
    python -m dvc run \
        ...
        \
        poetry run \
            python smaller3d/datasets/preprocessing/semantic_kitti_preprocessing.py make_instance_database \
