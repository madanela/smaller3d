...
function preprocess() {
    poetry run \
        python smaller3d/datasets/preprocessing/scannet_preprocessing.py preprocess_sequential \
        --git_repo="$GIT_REPO" \
        --data_dir="$DATA_DIR" \
        --save_dir="$SAVE_DIR"
}
...
function make_instance_database() {
    poetry run \
        python smaller3d/datasets/preprocessing/scannet_preprocessing.py make_instance_database_sequential \
...
function preprocess_dvc () {
    python -m dvc run \
        -d smaller3d/datasets/preprocessing/scannet_preprocessing.py \
        ...
        \
        poetry run \
            python smaller3d/datasets/preprocessing/scannet_preprocessing.py preprocess \
            --git_repo="$GIT_REPO" \
            --data_dir="$DATA_DIR" \
            --save_dir="$SAVE_DIR"
        # -d $DATA_DIR \
...
function make_instance_database_dvc () {
    python -m dvc run \
        ...
        \
        poetry run \
            python smaller3d/datasets/preprocessing/scannet_preprocessing.py make_instance_database \
            --train_database_path="$SAVE_DIR/train_database.yaml" \
            --git_repo="$GIT_REPO" \
            --data_dir="$DATA_DIR" \
            --save_dir="$SAVE_DIR"
